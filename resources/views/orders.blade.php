...
<div class="container mt-5">
    ...
    <div> 
        <a href='/creation/orderCreationForm' class="btn btn-primary">Add Data</a>
    </div>
    </div>
    <div class="card-body ">
        @if(session('created'))
        ...
        @foreach($data as $item)
            <tr >
                ...
                <td class="truncate text-center">{{$item->customer_Id}}</td>
                <td class="truncate text-center">{{$item->TotalPrice}}</td>
                <td class="truncate text-center">{{$item->TotalAmount}}</td>
                <td class="truncate text-center">{{$item->status}}</td>
                <td class="truncate text-center">{{$item->created_at}}</td>
                <td class="truncate text-center">{{$item->updated_at}}</td>

                <td class="d-flex justify-content-center" >
                <a href={{url('/show/orderShow/'.$item->id)}} class="btn btn-success mr-2"><i class="fa-solid fa-eye fa-beat"></i></a></i>
